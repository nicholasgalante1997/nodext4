...
describe('BlockDevice', () => {
  ...
  describe('init', () => {
    it('should initialize successfully for valid block device', async () => {
      ...
      const mockIsBlockDevice = spyOn(BlockDevice, 'isBlockDevice').mockResolvedValue(true);
    ...
    it('should throw error for non-block device', async () => {
      ...
      const mockIsBlockDevice = spyOn(BlockDevice, 'isBlockDevice').mockResolvedValue(false);

      await expect(device.init()).rejects.toThrow('Path /tmp/regular-file is not a block device.');
  ...
  describe('getAbsoluteSizeOfBlockDevice', () => {
    ...
    it('should throw error when device not initialized', () => {
      ...
      expect(() => device.getAbsoluteSizeOfBlockDevice()).toThrow('Block device not initialized.');
    });
